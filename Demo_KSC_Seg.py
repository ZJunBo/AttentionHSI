...
from autis import *
from Model.model_KSC import SwinHSI
...
for (FLAG,curr_train_ratio) in [(5, 0.05)]:
    ...
    data = np.reshape(data, [m, n, d]) # [512,614,176]
    ...
    model = SwinHSI(num_classes=14, n_bands=176, dim=96)
    print(model)
    ...
    for eep in range(max_epoch):
        ...
        if eep%10==0:
            ...
            #                             'ResultsImage/' + dataset_name + str(train_ratio) + '_' + str(
            #                                 OA) + '_' + str(AA) + '_' + str(kappa))
