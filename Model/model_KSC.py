...
from Model.KSC_cat import SwinTransformer
import math
...
class SwinHSI(nn.Module):
   def __init__(self, num_classes, n_bands , dim):  #  num_classes为数据集中的类别数  n_bands为光谱段
        ...
        #encoder conv gn relu
        # self.layer2_conv = nn.Conv2d(2 * dim, 2 * dim, 3, 1, 1)
        # self.layer2_gn = nn.GroupNorm(6, 2 * dim)
        # self.layer2_relu = nn.ReLU(inplace=True)
        #
        # self.layer4_conv = nn.Conv2d(8 * dim, 8 * dim, 3, 1, 1)
        # self.layer4_gn = nn.GroupNorm(24, 8 * dim)
   ...
   def forward(self, x):  # x [1, 176, 181, 215]
       ...
       score1 = self.score1_relu(score1) # 10为通道数， 6为groups数量  [1, 192, 12, 14]
